<?php
	 ...
	 */
	function gmt_donation_invoices_set_is_paid( $post ) {
		if ( get_post_type( $post->ID ) !== 'gmt_donate_invoices' ) return;
		update_post_meta( $post->ID, 'gmt_invoice_paid', false );
		update_post_meta( $post->ID, 'gmt_invoice_payment_method', false );
	}
